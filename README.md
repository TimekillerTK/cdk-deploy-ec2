...
* [venv](https://docs.python.org/3/library/venv.html) or [pyenv-virutalenv](https://github.com/pyenv/pyenv-virtualenv)
...
## How to use
Once you have installed all the prequisites:
1. Clone this repository and change to the directory
    ```
    git clone https://github.com/TimekillerTK/cdk-deploy-ec2.git
    cd cdk-deploy-ec2
    ```
2. Create a new python virtual environment with `venv` or `pyenv-virtualenv` and **activate it**
   * `venv` 
    ```
    python -m venv .venv
   ...
   * `pyenv-virtualenv`
    ```
    pyenv virtualenv 3.10.5 cdk-deploy-ec2
    pyenv local cdk-deploy-ec2
    ```
3. Install project dependencies with `poetry`
    ```
    poetry install
    ```
4. Run `cdk deploy` with poetry to deploy your stack
    > ⚠️ You need to set some environment variables first, see [here](#setting-environment-variables) for examples.
    ```
